...
In the eAOO paper, we report that for GWAS123,
...
and 1,431 late here vs. 1,467 in our earlier analysis); however, the mean
age of onset for each group is rather different: we find mean eAOO of 6.7 here
...
histogram_with_contours <- ggplot(case_only_analysis_df, aes(x = VITageonset)) +
  ...
  labs(title = "Vitiligo Age of Onset Histogram") +
  theme_minimal() +
...
\newpage
## Association of each PRS with vitiligo risk
...
predictors <- c(scaled_prs_cols, "eA00_rs145954018", "generic_rs9271597")
...
The pvals = 0 mean that the p-value is lower than the numerical precision in R,
...
Again, these results look about as I would expect: the MHC class 2 only PRS
