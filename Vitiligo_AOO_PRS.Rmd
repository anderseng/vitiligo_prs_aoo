...
## Compare Top 1 Percent PRS in Early vs Late Onset
```{r}
analysis_df <- analysis_df %>%
  mutate(no_mhc_classII_prs_category =
           case_when(
             no_mhc_classII_prs_percentile >= 0.99 ~ "High PRS",
             TRUE ~ "Normal PRS"
           )) %>%
...
print(str_c("Fisher's Exact P-value: ", round(fisher_p_value, 2)))
...
First, downsample cases so that there are exactly 700 cases with early-onset and
700 cases with late-onset. Then, compute the association between each of the 
...
##make the plot
plot2 <- ggplot(filter(pivot_eaoo_laoo_all_snps,
              !SNP %in% c("RS145954018", "RS114448410")),
...
plot2
ggsave(str_c(here(), "/figures/fig2.eps"), height = 3, width = 6, units = "in")
ggsave(str_c(here(), "/figures/fig2.tiff"), height = 3, width = 6, units = "in")
```
...
mean_data <- yoo_only_df_filt %>%
  group_by(period) %>%
  summarize(mean_prs = mean(no_mhc_classII_prs_scaled))

intercept <- prs_reg %>% filter(term == "(Intercept)") %>% pull(estimate)
slope <- prs_reg %>% filter(term == "period_ordinal") %>% pull(estimate)
pval <- prs_reg %>% filter(term == "period_ordinal") %>% pull(p.value)

# Plot
plot3 <- ggplot(yoo_only_df_filt, aes(x = period, y = no_mhc_classII_prs_scaled, fill = period)) +
  geom_violin(show.legend = FALSE) +
  geom_boxplot(width = 0.1, fill = "white", color = "black", show.legend = FALSE) +
  ...
  xlab("Time Period") +
  ylab(paste("Incident Vitiligo Case PRS",
  "(MHC Class II Excluded)", sep="\n")) +
  geom_text(aes(label = paste0("P-value = ", round(pval, 2))),
             x = max(yoo_only_df_filt$period), y = max(yoo_only_df_filt$no_mhc_classII_prs_scaled), hjust = 1, vjust = 1, size = 3, color = "black") +
  geom_text(aes(label = paste0("\u03B2 = ", round(slope,2))),
          x = max(yoo_only_df_filt$period), y = max(yoo_only_df_filt$no_mhc_classII_prs_scaled) - 0.4, hjust = 1, vjust = 1, size = 3, color = "black") +
  theme_bw()

plot3
ggsave(str_c(here(), "/figures/fig3.eps"), height = 3, width = 6, units = "in")
ggsave(str_c(here(), "/figures/fig3.tiff"), height = 3, width = 6, units = "in")
